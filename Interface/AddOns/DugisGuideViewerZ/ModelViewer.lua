...
function MV:Initialize()
	...
	local function ToggleModelPage(direction)
		...
		elseif modelType == "OBJ" and MV.objDB[modelId] then
			MV.Frame.model:SetModel(MV.objDB[modelId])
	...
	function MV.OnUpdate(self, elapsedTime) 
		...
		if MV.moving then
			...
			if theShortestDistance < 20  then
				LuaUtils.ClearAndSetPoint(MV.Frame, relatedMgnetPair[2], SmallFrameBkg, relatedMgnetPair[1], 0, 0)
			...
			if distance > 30 then
				...
				LuaUtils.ClearAndSetPoint(MV.Frame, "TOPLEFT", UIFrame, "TOPLEFT", mX - startMousePointOnMVFrame[1], -(mouseY - startPosY))
			end
		end
	...
	function MV:Load()
		...
		function MV:RegisterObjects(objFunc)
			...
			for objID, modelID in pairs(table) do
				self.objDB[tostring(objID)] = modelID
			end
		end
		...
		function MV:RegisterNPCs(npcFunc)
			...
			lineEnd = 0
			lineStart = 0
			while (lineStart and safetyCount < 30000) do
