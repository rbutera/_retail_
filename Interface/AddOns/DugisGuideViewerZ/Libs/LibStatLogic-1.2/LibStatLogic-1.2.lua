...
function StatLogic:SetTip(item)
	local name, link, _, _, reqLv, _, _, _, itemType = LuaUtils.GetItemInfo_dugi(item)
	if not link then
...
--]]
local function CNumberEx(value, base, localization)
	--[[
	if (value == nil) then
...
local function CNumber(value, base)
...
if playerArmorType then
	...
	local function UpdateArmorSpecActive()
		...
		for _, slot in pairs(WatchInventoryID) do
			...
			if (select(7, LuaUtils.GetItemInfo_dugi(item))) ~= playerArmorType then return end
		end
		-- all pass
...
local function IsMetaGemActive(item)
	...
	local name, link, _, _, _, _, itemSubType = LuaUtils.GetItemInfo_dugi(item)
...
function StatLogic:RemoveExtraSocketGem(link)
	...
	local _, _, _, _, _, _, _, _, itemType = LuaUtils.GetItemInfo_dugi(link)
...
function StatLogic:GetGemID(item)
  ...
  -- Check if item is in local cache
  local name, link = LuaUtils.GetItemInfo_dugi(item)
  if not name then
  ...
  if not LuaUtils.GetItemInfo_dugi(6948) then -- Hearthstone
...
local function ItemIsTypeAndSubtype(item, isTypeIndex, isSubTypeIndex)
	local isLocalized = select(isSubTypeIndex,C_AuctionHouse.GetAuctionItemSubClasses(isTypeIndex))
	local itemLocalized = select(7, LuaUtils.GetItemInfo_dugi(item))
	return isLocalized==itemLocalized
end

local function IsRedRequiresLine(text, r, g, b)
    --checking for red color
    if math.abs(r - 0.99999779462814) <0.001 and math.abs(g - 0.12548992037773)<0.001  and math.abs(b - 0.12548992037773) < 0.001 then
        text = text:gsub(" ", "")
        local requires_what = {strsplit(":", text)}
        
        --For zhCN
        if #requires_what == 1 then
            requires_what = {strsplit("：", text)}
        end
        
        if #requires_what >= 2 then
            local requires = strtrim(requires_what[1])
            if requires == L.Requires then return true end
        end
    end
end

--This function only checks if in the item tooltip there a red "Requires:.."  line. In case this line is present
--we can be sure there would be a notification "Cant use thisitem" shown after attempt of using this gear.
function StatLogic:CanUseGear(link, threadingMode)
	local name, link = LuaUtils.GetItemInfo_dugi(link, threadingMode)
	...
	for i = 2, tip:NumLines() do
        ...
        if IsRedRequiresLine(text, r, g, b) then
           return false
        end
	end 
...
--This function only checks if in the item tooltip there a red "Requires:.."  line. In case this line is present
--we can be sure there would be a notification "Cant use thisitem" shown after attempt of using this gear.
function StatLogic:GetRequiredPlayerLevel(link, threadingMode)
	local name, link, _, _, reqLv = LuaUtils.GetItemInfo_dugi(link, threadingMode)
	if not name then return true end
	...
	for i = 2, tip:NumLines() do
		local text = tip[i]:GetText();
		local level = text:match( gsub(ITEM_MIN_LEVEL,"%%d","(.*)"))
...
function StatLogic:GetSum(item, table, threadingMode)
	...
	-- Check if item is in local cache
	local name, link, rarity , ilvl, reqLv, _, armorType, _, itemType = LuaUtils.GetItemInfo_dugi(item, threadingMode)
...
function StatLogic:GetFinalArmor(item, text)
	...
	-- Check if item is in local cache
	local name, _, rarity , ilvl, _, _, armorType, _, itemType = LuaUtils.GetItemInfo_dugi(item)
	if not name then return end
...
function StatLogic:GetDiffID(item, ignoreEnchant, ignoreGem, red, yellow, blue, meta, ignorePris)
	...
	-- Check if item is in local cache
	name, link, _, _, _, _, _, _, itemType = LuaUtils.GetItemInfo_dugi(item)
	if not name then return end
	...
	if itemType == "INVTYPE_WEAPON" then
		...
		if IsUsableSpell(GetSpellInfo(674)) then        -- ["Dual Wield"]
			local _, _, _, _, _, _, _, _, eqItemType = LuaUtils.GetItemInfo_dugi(linkDiff1)
