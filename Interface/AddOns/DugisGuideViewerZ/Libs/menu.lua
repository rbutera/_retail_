...
function ArrowMenu:CreateFrame(parent)
	...
	if free_frames > 0 then
		...
		frame = CreateFrame("Button", "DugisArrow_RecycledFrame"..frame_count, parent, "BackdropTemplate");
  	end
...
local function Menu_ShowAtCursor(self, auto_release)
  ...
  -- JU Removed autorelease which was caousing problem on UI Scale different than 1
  auto_release = false --(auto_release == nil) and true or auto_release
...
function ArrowMenu:CreateMenu(name)

  if menus[name] then
    --Depopulating menu items if some exist
    while #menus[name].items > 0 do
      local item = table.remove(menus[name].items)
      DugisGuideViewer.ArrowMenu:ReleaseMenuItem(item)
      menus[name].close_item = false
    end
