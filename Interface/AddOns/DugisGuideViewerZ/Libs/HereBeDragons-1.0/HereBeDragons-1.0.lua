...
if not oldversion or oldversion < 33 then
    ...
    local function processZone(id)
        ...
        if #floors == 0 and LuaUtils.DGV.GetCurrentMapDungeonLevel() > 0 then
            floors[1] = LuaUtils.DGV.GetCurrentMapDungeonLevel()
            mapData[id].fakefloor = LuaUtils.DGV.GetCurrentMapDungeonLevel()
        end
        ...
        for i = 1, mapData[id].numFloors do
            ...
            local _, right, bottom, left, top = LuaUtils.DGV.GetCurrentMapDungeonLevel()
...
local function UpdateCurrentPosition()
    ...
    local prevContinent
    local prevMapID, prevLevel = GetCurrentMapAreaID(), LuaUtils.DGV.GetCurrentMapDungeonLevel()
    ...
    local newMapID, newLevel = GetCurrentMapAreaID(), LuaUtils.DGV.GetCurrentMapDungeonLevel()
