...
local MAJOR, MINOR = "HereBeDragons-Pins-2.0-Dugis", 6
...
local function UpdateMinimapPins(force)
    ...
    if x ~= lastXY or y ~= lastYY or diffZoom or facing ~= lastFacing or force then
        ...
        for pin, data in pairs(minimapPins) do
            if data.showEveryWhere or (data.instanceID == instanceID and (not data.uiMapID or data.uiMapID == mapID or (data.showInParentZone and IsParentMap(data.uiMapID, mapID)))) then
...
WorldMapFrame.pinPools["HereBeDragonsPinsTemplateDugi"] = worldmapPinsPool

-- provider base API
function worldmapProvider:RemoveAllData()
    self:GetMap():RemoveAllPinsByTemplate("HereBeDragonsPinsTemplateDugi")
end

function worldmapProvider:RemovePinByIcon(icon)
    for pin in self:GetMap():EnumeratePinsByTemplate("HereBeDragonsPinsTemplateDugi") do
...
function worldmapProvider:HandlePin(icon, data)
    ...
    if uiMapID == WORLD_MAP_ID then
        ...
        if uiMapID ~= data.uiMapID and data.worldMapShowFlag ~= HBD_PINS_WORLDMAP_SHOW_WORLD and data.worldMapShowFlag ~= HBD_PINS_EVERYWHERE then 
			return 
		end
        ...
        if uiMapID ~= data.uiMapID then
            ...
            if not data.uiMapID then
				...
				if data.worldMapShowFlag == HBD_PINS_EVERYWHERE then
					show = true
				end
    ...
    if x and y then
        self:GetMap():AcquirePin("HereBeDragonsPinsTemplateDugi", icon, x, y, data.frameLevelType)
    end
end
...
-- @param floatOnEdge flag if the icon should float on the edge of the minimap when going out of range, or hide immediately (default false)
function pins:AddMinimapIconMap(ref, icon, uiMapID, x, y, showInParentZone, floatOnEdge, showEveryWhere)
    if not ref then
    ...
    --This showEveryWhere parameter is added by DugisGuideViewer addon.
    minimapPins[icon].showEveryWhere = showEveryWhere
end
