...
local enabledExpansionAndNearbyCheck = false

DGV.questAutoGuideName = "Auto World & Daily Quest Guide"
...
function Guides:Initialize()
	...
	local tabs = {
		...
		[5] = {text = "Leveling", 	title = "Leveling Guides", 	guidetype = "L",	LeftFrame = DGVHomeFrame, 		RightFrame = DGVScrollFrame4,	icon="Interface\\Icons\\achievement_level_60", rightShouldScroll = false},
		...
		[10] = {text = "Achievements",	title = "Achievement Guides", guidetype = "A",LeftFrame = DGVHomeFrame,		RightFrame = DGVScrollFrame8,	icon="Interface\\Icons\\ACHIEVEMENT_GUILDPERK_HONORABLEMENTION", rightShouldScroll = false},
		[9] = {text = "Professions", 	title = "Profession Guides", guidetype = "P",LeftFrame = DGVHomeFrame,		RightFrame = DGVScrollFrame9,	icon="Interface\\Icons\\INV_Scroll_11", rightShouldScroll = false},
	...
	function DGV:ReloadCurrentGuide()
		DGV:DisplayViewTabInThread(CurrentTitle, nil, true)
        DGV:CollapseCurrentGuide()
        DGV:UpdateCurrentGuideExpanded()  
	end

	function DGV:ReloadButtonOnClick()
		DGV:ReloadCurrentGuide()
	end

	function DGV:ResetCurrentGuide(onEnd)
    ...
    function QuestSuperTracking_GetClosestQuest_Dugis()
        ...
        for i = 1, C_QuestLog.GetNumWorldQuestWatches() do
            ...
            if watchedWorldQuestID then
                ...
                if distanceSq and distanceSq <= minDistSqr then
                    if QuestUtils_IsQuestWatched(watchedWorldQuestID) then
                        minDistSqr = distanceSq;
                        closestQuestID = watchedWorldQuestID;
                        title_ = C_TaskQuest.GetQuestInfoByQuestID(closestQuestID)
                    end
                end
            end
        end
		...
		for i = 1, C_QuestLog.GetNumQuestWatches() do
			...
			local frequency = title.frequency;
			
			if questID then
				local distSqr, onContinent = C_QuestLog.GetDistanceSqToQuest(questID);
				if ( distSqr and distSqr <= minDistSqr ) then
					if QuestUtils_IsQuestWatched(questID) or frequency == LE_QUEST_FREQUENCY_DAILY or frequency == LE_QUEST_FREQUENCY_WEEKLY then
						minDistSqr = distSqr;
        ...
        if not closestQuestID then
			for questLogIndex = 1, C_QuestLog.GetNumQuestLogEntries() do
                ...
                if ( not info.isHeader and QuestHasPOIInfo(info.questID) ) then
	...
	local function EvaluateTID(tid)
		return tid and C_QuestLog.IsQuestFlaggedCompleted(tid) and not DGV:GetQuestLogIndexByQID(tid)
	end	
	...
	local function EvaluateAYG(ayg)
		...
		local aygComplete
		if index then 		
			aygComplete = C_QuestLog.IsComplete(C_QuestLog.GetInfo(index).questID);
		end
		return C_QuestLog.IsQuestFlaggedCompleted(ayg) or aygComplete
	end		
	
	local function EvaluateOID(oid)
		local index = DGV:GetGuideIndexByQID(tonumber(oid), "A")
		return (index and DGV:GetQuestState(index) == "C") or C_QuestLog.IsQuestFlaggedCompleted(oid)
	end		
	...
	local function EvaluatePRE(pre)
		...
		return (index and DGV:GetQuestState(index) == "C") or C_QuestLog.IsQuestFlaggedCompleted(pre)
	end
	...
	local function EvaluateFS_(factionId, standingRequirement)
		...
		if not barValue then 
			barValue = select(2, GetFriendshipReputation(tonumber(factionId))) or 1
		end				
		return barValue >= tonumber(standingRequirement)
	end
	...
	function DGV:RenownCompletedAtGuideIndex(guideindx)
		local requiredRenownLevel = DGV:ReturnTag("RENOWN", guideindx)

		if requiredRenownLevel then
			if C_CovenantSanctumUI.GetRenownLevel() >= tonumber(requiredRenownLevel) then
				return true
			end
		end
	end
	...
	function DGV:AskGuideSuggest(playerLevel)
		...
		if self:UserSetting(DGV_GUIDESUGGESTMODE) then
			...
			for guideInRange,minimum in IterateGuidesInRange do
				if minimum > 1 and rangeCount < 6 then
	...
	function DGV:ShouldSuggestNewGuide(playerLevel, checkIfWasAlreadySuggested)
		...
		--124 DK Start Zone, 179, 180, 181, 182, 202, Worgen Start Zone, 194, 174, 175, 176, 177, 178 Goblin Start Zone
		local mapId = DGV:GetCurrentMapID() 
		DebugPrint("Current mapID:"..mapId)
		if mapId == 124 or mapId == 179 or mapId == 180 or mapId == 181 or mapId == 182 or mapId == 194 or mapId == 202 or mapId == 174 or mapId == 175 or mapId == 176 or mapId == 177 or mapId == 178 then return end 
	...
	function DGV.ShowSuggestGuideNotification(playerLevelToBeUsed, onLevelUp)
        ...
        --CurrentTitle
        if DGV:ShouldSuggestNewGuide(playerLevel, DGV:UserSetting(DGV_ENABLED_GUIDE_NOTIFICATIONS)) 
        and (CurrentTitle == nil or DGV.gtype[CurrentTitle] == "L") and DGV:UserSetting(DGV_GUIDESUGGESTMODE) then
			if DGV:UserSetting(DGV_ENABLED_GUIDE_NOTIFICATIONS) then
	...
	function DGV:GetSuggestedGuide(playerLevel)
		...
		local startguides = 
		{
				BloodElf = "94(1-50 Blood Elf)#94(1-50 Blood Elf)#94(1-50 Blood Elf)", 
				Orc = "1(1-50 Orc)#1(1-50 Orc)#1(1-50 Orc)", 
				Troll = "1(1-50 Troll)#1(1-50 Troll)#1(1-50 Troll)", 
				Goblin = "194(1-20 Goblin)",
				Tauren = "7(1-50 Tauren)#7(1-50 Tauren)#7(1-50 Tauren)", 
				Scourge = "18(1-50 Undead)#18(1-50 Undead)#18(1-50 Undead)", 
				Undead = "18(1-50 Undead)#18(1-50 Undead)#18(1-50 Undead)", 
				Dwarf = "27(1-50 Dwarf)#27(1-50 Dwarf)#27(1-50 Dwarf)", 
				Gnome = "27(1-50 Gnome)#27(1-50 Gnome)#27(1-50 Gnome)", 
				Draenei = "97(1-10 Draenei)#97(1-10 Draenei)#97(1-10 Draenei)", 
				Human = "37(1-50 Human)#37(1-50 Human)#37(1-50 Human)", 
				NightElf = "57(1-50 Night Elf)#57(1-50 Night Elf)#57(1-50 Night Elf)", 
				Worgen = "179(1-20 Worgen)", 
				DeathKnight = "124(8-30 Death Knight)",
				Pandaren = "378(1-20 Pandaren)",
				DemonHunter = "672(8-30)",
				Level45 = "War Campaign (45-50)",
				Exile = "1409(1-10)",
				VoidElf = "84(10+ Allied Races)",
				LightforgedDraenei = "84(10+ Allied Races)",
				DarkIronDwarf = "84(10+ Allied Races)",
				Mechagnome = "84(10+ Allied Races)",
				KulTiran = "84(10+ Allied Races)", 
				Nightborne = "85(10+ Allied Races)",
				HighmountainTauren = "85(10+ Allied Races)",	
				MagharOrc = "85(10+ Allied Races)",
				ZandalariTroll = "85(10+ Allied Races)",	
				Vulpera = "85(10+ Allied Races)",
		}
		
		if playerLevel >= 45 and playerLevel < 50 then 
			guideName = startguides["Level45"]
		elseif currentZone == 1727 or currentZone == 1409 or currentZone == 1726 then
			guideName = startguides["Exile"]
	...
	function DGV:DelayAndMapCurrentObjective(delay, fromOnModulesLoaded) 
		if DelayAndMapCurrentObjectiveFrame:IsShown() then return end
		DelayAndMapCurrentObjectiveFrame.delay = delay 
		DelayAndMapCurrentObjectiveFrame.fromOnModulesLoaded = fromOnModulesLoaded 
	...
	DelayAndMapCurrentObjectiveFrame:SetScript("OnUpdate", function(self, elapsed) 
		...
		if self.delay <= 0 then  
			...
			DGV:WatchQuest()
			DugisArrowGlobal:RestoreWaypoints()
		end
	end)		
...
-- 					if i == #XYVals then TomTom:SetCrazyArrow(DGV.DugisArrow.waypoints[1].tomtom, 5, desc) DGV.DugisArrow:Hide() end
	...
	function DGV:WatchQuest()
		...
		local canRemoveQuests = CurrentTitle ~= DGV.questAutoGuideName
		
		--AWQ check if needed because Auto Guide is basing on currently watched quests. When removing the watch by RemoveQuestWatch 
		--the Auto Guide functionality is not working anymore reliably.
		if self:UserSetting(DGV_ENABLEQW) and DGV:isValidGuide(CurrentTitle) == true and canRemoveQuests then
			for logindex = 1, C_QuestLog.GetNumQuestLogEntries() do
				...
				if DGV.carboniteloaded and qid ~= 0 and Nx.Quest then 
					...
					C_QuestLog.RemoveQuestWatch(C_QuestLog.GetInfo(logindex).questID)
				end 
			end
		end
	...
	function DGV:havecurrencyitem(indx)
		...
		if curitem then
			currInfo = C_CurrencyInfo.GetCurrencyInfo(curitem)
		end

		if curitem and (currInfo and currInfo.quantity >= curqty) then havec = true else havec = false end
		return havec
	end	
    ...
    local function isQuestCompleted(qid)
        if qid then
			return C_QuestLog.IsComplete(qid);
        end
    end
	...
	function DGV:CheckForSkip(indx) 
		...
		elseif optional and tidInlog then
			return true
		elseif optional and action =="T" and not isQuestCompleted(questId) then
			return true															
	...
	function DGV:MoveToNextQuest(MoveToIndex, isInThread, recursion, fromOnModulesLoaded, fromSeverUpdate)
		local oldIndex = DGU.CurrentQuestIndex
		local checkMoved
		...
		if DGU.CurrentQuestIndex <= lastUsedStepIndex then
            ...
            --First two conditions are to prevent infinite loop / stack overflow
			if DGU.CurrentQuestIndex and oldQuestIndex ~= DGU.CurrentQuestIndex and (DGV:havelootitem(DGU.CurrentQuestIndex) == true or DGV:havecurrencyitem(DGU.CurrentQuestIndex) == true) then
				DebugPrint("#####havelootitem(DGU.CurrentQuestIndex) ")
				DGV:SetChkToComplete(DGU.CurrentQuestIndex)
				DGV:MoveToNextQuest(nil, isInThread, true)    
			end	
	...
	function DGV:DisplayViewTab(title, skip, threading, autoScrollToCurrent, fromOnModulesLoaded)
		...
		if title == nil or DGV:isValidGuide(title) == false then
			...
			local dayOfTheYear = LuaUtils:GetDayOfTheYear()
		
			if DGU.dayOfTheYear ~= dayOfTheYear and DGU.dayOfTheYear ~= nil then
				Guides:OnDayChange(threading) 
			end
	
			DGU.dayOfTheYear = dayOfTheYear
    ...
    function DGV:DisplayViewTabInThread(title, skip, autoScrollToCurrent)
	...
	function DGV:UpdatePercentText(guideTitle, percentText, gtype, threading)
		...
		if guidesize and guidesize > 0 then
	...
	local function SetCurrentGuideTabPercentComplete()
		if not DGV.gtype then return end
	...
	function DGV:Skip(qid)
		...
		for guideIndex = 1, lastUsedStepIndex do
			if qid and (DGV.qid[guideIndex] == qid) and (self:GetQuestState(guideIndex) ~= "C") and strmatch(self.actions[guideIndex], "[ACTNK]") then 
	...
	function DGV:getIcon(objectiveType, i)
		...
		--[[ Not working correctly it overwrites a lot icon, probably dont need it
		...
		elseif isDungeon then
			...
			return button.validTexture	
		elseif isAlchemy and not button.validTexture then
			button.validTexture = "Interface\\Icons\\Trade_Alchemy" 
			return button.validTexture
		elseif isBlacksmith and not button.validTexture then
			button.validTexture = "Interface\\Icons\\Trade_BlackSmithing" 
			return button.validTexture
		elseif isCooking and not button.validTexture then
			button.validTexture = "Interface\\Icons\\inv_misc_food_15" 
			return button.validTexture
		elseif isDisenchant and not button.validTexture then
			button.validTexture = "Interface\\Icons\\inv_enchant_disenchant" 
			return button.validTexture
		elseif isEnchanting and not button.validTexture then
			button.validTexture = "Interface\\Icons\\trade_engraving"
			return button.validTexture
		elseif isEngineering and not button.validTexture then
			button.validTexture = "Interface\\Icons\\Trade_Engineering"
			return button.validTexture
		elseif isFishing and not button.validTexture then
			button.validTexture = "Interface\\Icons\\Trade_Fishing"
			return button.validTexture
		elseif isFirstaid and not button.validTexture then
			button.validTexture = "Interface\\Icons\\Trade_Fishing"
			return button.validTexture
		elseif isHerb and not button.validTexture then
			button.validTexture = "Interface\\Icons\\Trade_Herbalism" 
			return button.validTexture
		elseif isInscription and not button.validTexture then
			button.validTexture = "Interface\\Icons\\inv_inscription_tradeskill01" 
			return button.validTexture
		elseif isJewe and not button.validTexturel then
			button.validTexture = "Interface\\Icons\\inv_misc_gem_01" 
			return button.validTexture
		elseif isLeather and not button.validTexture then
			button.validTexture = "Interface\\Icons\\Trade_LeatherWorking" 
			return button.validTexture		
		elseif isMining and not button.validTexture then
			button.validTexture = "Interface\\Icons\\Trade_Mining" 
			return button.validTexture
		elseif isSkinning and not button.validTexture then
			button.validTexture = "Interface\\Icons\\inv_misc_pelt_wolf_01"
			return button.validTexture
		elseif isSmelting and not button.validTexture then
			button.validTexture = "Interface\\Icons\\spell_fire_flameblades" 
			return button.validTexture		
		elseif isTailoring and not button.validTexture then
		...
		elseif isLoot then --Use (item:x) icon instead
			--button.validTexture = icontbl[5].path
			if qid then 
				button.tag_id = qid
				button.tagType = "qid"
			...
			return button.validTexture		
		elseif (objectiveType == "N" ) and not button.validTexture then  
			button.validTexture = icontbl[11].path
			if qid then 
				button.tag_id = qid
				button.tagType = "qid"
			...
			return button.validTexture		
		elseif not button.validTexture then -- (objectiveType == "K" ) then  
			--return "Interface\\Minimap\\TRACKING\\Ammunition"
			button.validTexture = icontbl[4].path
			if qid then 
				button.tag_id = qid
	...
	function DGV:ParseRows(threading, _, infoOnly, _,...)
		...
		local questTitleAndType2questIds_temp = {}
		...
		local activeCovenant	= C_Covenants.GetActiveCovenantID() 
        ...
        if not infoOnly then
            if DGV.actions then wipe(DGV.actions) end
            if DGV.quests1 then wipe(DGV.quests1) end
            if DGV.quests1L then wipe(DGV.quests1L) end
            if DGV.quests2 then wipe(DGV.quests2) end
            if DGV.aqids then wipe(DGV.aqids) end
            lastUsedStepIndex = -1
        end
		...
		for i = 1, select ("#", ...) do
			...
			local _, _, covenant		= text:find("|COV|([^|]+)|")
			local _, _, covenantRange		= text:find("|COV|CovID([^|]+)|")
			...
			if text ~= "" then
				...
				local isMatchingCovenant = true

				if covenantRange then
					covenantRange = string.gsub(covenantRange, "%(", "")
					covenantRange = string.gsub(covenantRange, "%)", "")
			 		local min_max =LuaUtils:split(covenantRange, "-")

					if not (activeCovenant >= tonumber(min_max[1]) and activeCovenant <= tonumber(min_max[2])) then
						isMatchingCovenant = false
					end 
				else
					if covenant then
						if tonumber(covenant) ~= activeCovenant then
							isMatchingCovenant = false
						end
					end
				end
				...
				if 
				(not classes or classes:find(myClass)) 
				and (not races or races:find(myRace)) 
				and (not gender or myGender == gender) 
				and isMatchingCovenant
				and (not faction or myFaction == faction) then

					if action and quest then 
						if type(infoOnly) == "string" and daily --this last "daily" condition needs to be removed in case infoOnly has more variants
						then
							local isDaily_index = LuaUtils:split(infoOnly, ":")
 
							if isDaily_index[1] == "isDaily" and tonumber(isDaily_index[2]) == indx then
								return true
							end
						end
				...
				if qid then
					local key = quest..action
					local questIds = questTitleAndType2questIds_temp[key]
					if not questIds then
						questIds = {}
						questTitleAndType2questIds_temp[key] = questIds
					end
					questIds[qid] = true
				end
			end
		...
		if DGV.quests1 then
			for index, questText in pairs(DGV.quests1) do
				local action =  DGV.actions[index]
				local key = questText..(action or "")

				local questIds = questTitleAndType2questIds_temp[key]
				if questIds then
					for questId, questText in pairs(questIds) do
						local aqids = DGV.aqids[index]
						if not aqids then 
							aqids = {}; 
							DGV.aqids[index] = aqids 
						end
						aqids[questId] = true
					end
				end
    ...
    function DGV:UpdateRowVisualizations(i)
		...
		if rowObj.currentTitle ~= CurrentTitle or rowObj.needsUpdate then
            local qDesc = DGV:GetQuestStepText(i) or ""
            ...
            if not self:HasCoord(i) then frame.WayPoint:Disable() else frame.WayPoint:Enable() end
	...
	function DGV:SetQuestText( i ) 
		...
		local row =  visualRows[i]
		local rowFrame = row and row.frame
    ...
    function DGV:GetFlatternRecentGuides()
		local result = {}
		if DugisGuideUser.RecentGuides and DugisGuideUser.RecentGuides.Categories then 
    ...
    function DGV:AddGuideToRecentGuides(rawGuideTitle)
        ...
        if not ExistsInRecentGuides(rawGuideTitle) and DugisGuideUser.RecentGuides then
		...
		if gtype=="I" or gtype=="L" then
			for guideInRange,minimum in IterateGuidesInRange,nil,key do
				if DGV.gtype[guideInRange] == gtype then
					local passedByExpansionCheck = Guides:GuideMatchesExpansion(guideInRange)

					if gtype=="I" or passedByExpansionCheck then
						return guideInRange
					end
				end
			end
    ...
    function Guides.UpdateSuggestedQuests(threading)
        ...
        for i=1, C_QuestLog.GetNumQuestLogEntries() do
            local qid2 = C_QuestLog.GetInfo(i).questID
	...
	function Guides:Load()
        ...
        DGV:RegisterEvent("COVENANT_CHOSEN")		
        ...
        --Extra quest ids
		DGV.eqids = {}
		--[[All quest ids related to the same quest. Used by Share Guide system. 
		It is needed for case when player with class A wants to share the guide with player with class B. 
		Sometimes quests have different numbers - for example when players have different classes. 
		A good example: 01_10_Elwynn_Forest_Human.lua
		Example:
			{[1]={[123]=true, [456]=true}, [2]={[789]=true, [654]=true}}
		]]
		DGV.aqids = {}
        DGV.sid = {}
		DGV.daily = {}
		...
		local function UpdateAbandonQID()
			...
			AbandonQID = C_QuestLog.GetSelectedQuest()
			--DGV:DebugFormat("UpdateAbandonQID", "AbandonQID", AbandonQID)
		end
        
		abandonQuestReaction = DGV.RegisterMethodReaction(C_QuestLog, "AbandonQuest", nil, UpdateAbandonQID)
		...
		function DGV:UpdateMainFrame(isInThread)
			...
			for guideIndex = 1, lastUsedStepIndex do
				...
				if logIndx then qComplete = C_QuestLog.IsComplete(qid) end
                ...
                if eqids then
                    for _, questId_ in pairs(LuaUtils:split(eqids, " ")) do
                        ...
                        if questId_ then
							...
							local qComplete_ = C_QuestLog.IsComplete(questId_);
							if qComplete_ ~= true then
								qComplete = false
							end
                        end
                    end
                ...
                or self:RenownCompletedAtGuideIndex(guideIndex)
			...
			DugisGuideViewer.Guides.UpdateVisuals()
		end
		...
		function DGV:UpdateAchieveFrame()
			if DGV:isValidGuide(CurrentTitle) == true then
				if DGV.gtype[CurrentTitle] == "A" or DGV.gtype[CurrentTitle] == "E" or DGV.gtype[CurrentTitle] == "D" or self.gtype[CurrentTitle] == "L" then --achieve guide type
					local i
					for i=1, lastUsedStepIndex do
		...
		function DGV:Guide_CRITERIA_UPDATE()
			if DGV.gtype[CurrentTitle] == "A" or DGV.gtype[CurrentTitle] == "E" or DGV.gtype[CurrentTitle] == "D" or DGV.gtype[CurrentTitle] == "L" then
				achieveDelayStart( ) 	
			end
		end
		...
		function DGV:GOSSIP_CLOSED()
			LuaUtils:Delay(1, function()
				DebugPrint("###GOSSIP_CLOSED", UnitInVehicle("player"), UnitOnTaxi("player"))
				if self:ReturnTag("V", DGU.CurrentQuestIndex) or DGV.actions[DGU.CurrentQuestIndex] == "F" then
					if UnitInVehicle("player") == 1 
					or UnitControllingVehicle("player") == true 
					or UnitOnTaxi("player") == true 
					or HasOverrideActionBar("player") == true then
						DGV:SetChkToComplete(DGU.CurrentQuestIndex)
						DGV:MoveToNextQuest()
					end
				end
		...
		function DGV:COVENANT_CHOSEN()
			DGV:ResetCurrentGuide(function()
				DGV:ReloadCurrentGuide()
			end)
		end					
		...
		function DGV:ReturnTag(tag, i)
			...
			elseif tag == "CUR" then
                ...
                return unpack(val)
			elseif tag == "P" then
				local categoryID, maxlevel = tags:match("|P|(%d+)%s?(%d*)|")
                local val = {tonumber(categoryID), tonumber(maxlevel) or 1}
                cacheTable[tag] = val
                return unpack(val)
			elseif tag == "OP" then
				local categoryID, professionlvl = tags:match("|OP|(%d+)%s?(%d*)|")
                local val = {tonumber(categoryID), tonumber(professionlvl) or 1}
                cacheTable[tag] = val
                return unpack(val)
			elseif tag == "PL" then
	...
	function Guides:OnDayChange(threading) 
		for guideKey, state in pairs(DGU.QuestState) do
			if state ~= "U" then
				local guideTitle_rowIndex = LuaUtils:split(guideKey, ":")
				local guideTitle, rowIndex = guideTitle_rowIndex[1], guideTitle_rowIndex[2]
				rowIndex = tonumber(rowIndex)

				if DGV.gtype[guideTitle] == "D" then
					local isDaily = DGV:ParseRows(threading, guideTitle, "isDaily:"..rowIndex, string.split("\n","\n"..DGV.guides[guideTitle]()))
					if isDaily == true then
						DGU.QuestState[guideKey] = "U"
					end
				end
			end
		end
	...
	function Guides:GuideMatchesExpansion(guideTitle)
		if not enabledExpansionAndNearbyCheck then
			return true
		end

		local currentExpansionId = UnitChromieTimeID("player")
		local mapId2GuideTitles = Guides.ExpansionId2SuggestedGuides[currentExpansionId]
		if mapId2GuideTitles == nil then
			return true
		end

		local currentMapId = DGV:GetPlayerMapPositionDisruptive()

		local guideNames = mapId2GuideTitles[currentMapId]
		if guideNames and LuaUtils:isInTable(guideTitle, guideNames) then
			return true
		end

		local nearbyZones = MapUtilsData.MapId2NeighboringMapIds[currentMapId]
		for _, mapId_ in pairs(nearbyZones) do
			guideNames = mapId2GuideTitles[mapId_]
			if guideNames and LuaUtils:isInTable(guideTitle, guideNames) then
				return true
			end
		end
	...
	Guides.ExpansionId2SuggestedGuides = {
		[0] = { --The Cataclysm
			--Northern Barrens
			[50] = {"15(15-50)#15(15-50)#15(15-50)", "23(15-50)#23(15-50)#23(15-50)"}, 
			--Durotar
			[14] = {} 
		},
	}
