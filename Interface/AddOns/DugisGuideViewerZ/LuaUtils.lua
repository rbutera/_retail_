                    ...
                    for i=1, thread.resumeAmountPerFrame do
                        if coroutine.status(thread.thread) ~= "dead" and not UnitAffectingCombat("player") then
                            ...
                            if status=="dead" and result == false then
                                --assert(false, threadName_..":\n" .. message.."\Detailed stack:\n"..debugstack(thread.thread)) --creates nil value error sometime, need fixing
                            end
                        end
                    end
                else
    ...
    if not LuaUtils.DugiThreadFrame then
        ...
        LuaUtils.DugiThreadFrame:SetScript("OnUpdate" , function(self, elapsed)
			...
			if DGV and DGV.Modules then 
                ...
                if DGV.SmallFrame and DGV.SmallFrame.UpdateSmallFrameHeader  then
                    DGV.SmallFrame.UpdateSmallFrameHeader()
                end
			end
...
function LuaUtils:NamedDelay(delayName, timeSec, function_)
    if not namedTimers[delayName] then
...
function LuaUtils:PostCombatRun(name, function_)
    ...
    LuaUtils:QueueThread("dugi-post-combat-invoke", function()
...
function LuaUtils:IsElvUISkinEnabled(skinName)
    if ElvUI then
        local ElvUI_ = unpack(ElvUI);
        if ElvUI_.private.skins.blizzard.enable and ElvUI_.private.skins.blizzard[skinName] then 
            return  true
        end
    end
end  
...
function LuaUtils:PlaySoundFile(...)
    --Needs to be uncommented once PlaySoundFile is working fine back
    return PlaySoundFile(...)
end

--This function is not fully accurate. It assums that each month has 31 days. It can be used only to detect day change.
--To make this function 100% accurate needs to be taken into account for example February that has amount of days depending on year. 
function LuaUtils:GetDayOfTheYear()
    local d = C_DateAndTime.GetCurrentCalendarTime()
    return (d.month - 1) * 31 + d.monthDay 
end

function LuaUtils.ArePointsTheSame(a, b)
    return math.abs(a-b) < 0.001
end

--Optimized version of SetPoint
function LuaUtils.ClearAndSetPoint(frame, ...)
    local shouldUpdatePoint = false

    if frame:GetNumPoints() == 1 then
        local point, relativeTo, relativePoint, ofsx, ofsy = ...

        --Case:  obj:SetPoint(point, ofsx, ofsy);
        if tonumber(relativeTo) and tonumber(relativePoint) then
            ofsx, ofsy = relativeTo, relativePoint
            relativeTo = frame:GetParent()
            relativePoint = point
        end

        ofsx, ofsy = ofsx or 0, ofsy or 0

        local a2, b2, c2, d2, e2 = frame:GetPoint(1)
        if point ~= a2 or relativeTo ~= b2 or relativePoint ~= c2 or not LuaUtils.ArePointsTheSame(ofsx , d2 ) 
        or not LuaUtils.ArePointsTheSame(ofsy , e2 ) then
