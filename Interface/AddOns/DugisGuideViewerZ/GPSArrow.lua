...
local terrainMapPath2MapId = terrainMapPath2MapId

local noTerrainMinimapMapIds = {201, 204, 205, 1220, 905, 882, 885, 830, 572, 948}
...
function GPS.UpdateParentAnchor()
	if not DGV.chardb then
		return
	end

	if DGV:UserSetting(DGV_GPS_MERGE_WITH_DUGI_ARROW) and not DugisArrowGlobal:IsEnabled3DArrow() then
		if GPSArrowScroll and DGV.wayframe then
			LuaUtils.ClearAndSetPoint(GPSArrowScroll, "CENTER", DGV.wayframe, "CENTER", 0, 0)
		end
	else
		if GPSArrowScroll then
			...
			if x and y then
				...
				LuaUtils.ClearAndSetPoint(GPSArrowScroll, "TOPLEFT", UIParent, "TOPLEFT", x, -y)
			else
				LuaUtils.ClearAndSetPoint(GPSArrowScroll, "CENTER", UIParent, "CENTER", 0, 0)
			end
		end
	end
end
...
GPS.GetWorldMapSize = function()
	...
	if mapId then 
		info = C_Map.GetMapArtLayers(mapId)
	end

	if info and info[1] then
		return info[1].layerWidth, info[1].layerHeight
	end
...
function GPS.UpdateSize()
    ...
    if DugisArrowGlobal and DugisArrowGlobal.UpdateProgressBarPosition then
...
function GPS:Initialize()
	...
	function GPS.DrawQuestBlobs()
		...
		local questID = QuestMapFrame_GetDetailQuestID() or C_SuperTrack.GetSuperTrackedQuestID()
	...
	function GPS.RefreshPOIs()
		...
		GPS.WorldMapPOIFrame_SelectPOI(C_SuperTrack.GetSuperTrackedQuestID())
	...
	function GPS.UpdateTitlePosition()
		...
		GPS.title:SetPoint("TOP", GPSArrowScroll, "BOTTOM", 0, 0 - 10)
	end
	...
	function GPS.RemoveFogForNewMaps()
		...
		if exploredMapTextures then
			local  layerIndex =  WorldMapFrame.ScrollContainer.zoomLevels and WorldMapFrame.ScrollContainer:GetCurrentLayerIndex() or 1;
			local layers = {}
			if mapID then
	...
	function GPS.Zoom(scale)
		...
		GPSArrowScroll:SetClampRectInsets(0, 0, 20, 0)
	...
	function GPS.FixTopPosition()
		...
		if top < -10 then
			if DGV:UserSetting(DGV_GPS_MERGE_WITH_DUGI_ARROW) then
				...
				LuaUtils.ClearAndSetPoint(GPSArrowScroll, point, relativeTo, relativePoint, xOfs, -diff)
			else
				local point, relativeTo, relativePoint, xOfs, yOfs = GPSArrowScroll:GetPoint(1)
				yOfs = -20
				LuaUtils.ClearAndSetPoint(GPSArrowScroll, point, relativeTo, relativePoint, xOfs, yOfs)
			end
		end
	end
	...
	function GPS.UpdateMerged(resetPosition)
		if DGV:UserSetting(DGV_GPS_MERGE_WITH_DUGI_ARROW) then
			...
			local x, y =  (DGV.wayframe:GetLeft() or 0), (GetScreenHeight() or 0) - (GPSArrowScroll:GetTop() or 0)
			
			if resetPosition then
				y = (GetScreenHeight() or 0) - (DGV.wayframe:GetTop() or 0) - 25
			end
			
			local xShift =  DGV.wayframe:GetWidth() + 80
			
			if x >  (GetScreenWidth() or 0) * 0.5 then
				xShift = -w - 80
			end
			
			LuaUtils.ClearAndSetPoint(GPSArrowScroll, "TOPLEFT", UIParent,"TOPLEFT" , x + xShift, -y)
	...
	function GPS.TryCreatingWorldQuestPOIDD(info, taskIconIndex)
		...
		local data = C_QuestLog.GetQuestTagInfo(info.questId)

		if not data then
			return nil
		end

		local _, _, _, rarity, _, _ = C_QuestLog.GetQuestTagInfo(info.questId)

		local taskPOI = GUIUtils:GetTaskPOI(taskIconIndex, GPS.TaskPOIArray, GPSArrowPOI)
        ...
        if WorldMap_SetupWorldQuestButton then
            WorldMap_SetupWorldQuestButton(taskPOI, data.worldQuestType, rarity, data.isElite, data.tradeskillLineID, inProgress, selected, isCriteria, isSpellTarget)
        else
			QuestUtil.SetupWorldQuestButton(taskPOI, data, inProgress, isSuperTracked, nil, nil, false);
		end        		
	...
	local excludedMaps_ = {}
	for _, v in pairs(noTerrainMinimapMapIds) do 
		excludedMaps_[v] = true 
		local children = C_Map.GetMapChildrenInfo(v)
		if children then
			for _, info in pairs(children) do
				excludedMaps_[info.mapID] = true
			end
		end
	end
	...
	function GPS.UpdateMinimapTextures()
		...
		local TERRAIN_PATH_SHORT = '%s/map%02d_%02d'
		...
		if not noTerrainMap and not IsIndoors() and not excludedMaps_[uiMapID] then
				...
				for y=1,numTilesY do
					for x=1,numTilesX do
						...
						if playerIn then
							...
							local texturePath = TERRAIN_PATH_SHORT:format(DGV.TERRAIN_MAPS[terrainMapID], iTileX+(x-1), iTileY+(y-1))
							local textureId = terrainMapPath2MapId[texturePath]

							texture:SetTexture(textureId, false)
	...
	GPS.UpdateFrameSize = function()
		if DGV:IsBigMap(GPS.GetMapIdForGPSMap()) then
			...
			if worldMapWidth == 0  and not WorldMapFrame:IsShown() then
				DGV.ForceOpenWorldMap()
				worldMapWidth, worldMapHeight = GPS.GetWorldMapSize()
			end
	...
	GPS.Load = function()
		...
		if DGV:UserSetting(DGV_GPS_MERGE_WITH_DUGI_ARROW) and DGV.wayframe then
