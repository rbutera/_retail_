...
function AQA:Initialize()
	...
	local function QUEST_LOG_UPDATE ()
		...
		local start_entry = C_QuestLog.GetLogIndexForQuestID(C_QuestLog.GetSelectedQuest())
		...
		for i = 1, num_entries do
			local questID = C_QuestLog.GetInfo(i).questID
			C_QuestLog.SetSelectedQuest(questID)
			local title  = C_QuestLog.GetInfo(i).title;
			local is_complete = C_QuestLog.IsComplete(questID);
	...
	local function GOSSIP_SHOW()
		...
		local i
 		...
 		local availableQuests = C_GossipInfo.GetNumActiveQuests() + C_GossipInfo.GetNumAvailableQuests()	
		
		if GossipFrame and GossipFrame.buttons then
			for i = 1, availableQuests do
				button = GossipFrame_GetTitleButton(i)
		...
		for i = firstAvailable, availableQuests do
			...
			if button and button:IsVisible() then
				...
				if self.completed_quests[text] and DugisGuideViewer:UserSetting(DGV_AUTOQUESTTURNIN) then
					button:Click()
				elseif not self.incomplete_quests[text] and DugisGuideViewer:UserSetting(DGV_AUTOQUESTACCEPT) then
					button:Click()
				end
			end
		end
