...
local function AutoroutinesOnUpdate()
	if autoroutines then
		...
		--This is needed to prevent situation when the autoroutines at the end of the table are never processed because of timeout (THEORETICAL_TIME_LIMIT).
		--The bug "Fix taxi bug in Draenor Garrison" was caused by the timeout
		LuaUtils:ShuffleTable(autoroutines)
...
local function RegisterMethodReaction(object, functionName, predicate, action, ...)
	if not functionHookRegistry then functionHookRegistry = {} end
	if not functionHookRegistry[functionName] then
		hooksecurefunc(object, functionName, 
			function(...)
				TryInvokeReactions(functionName, ...)
			end)
		functionHookRegistry[functionName] = true
...
DGV.RegisterMethodReaction = RegisterMethodReaction
