...
function TaxiDB:Initialize()
					...
					if WorldFlightMapFrame == nil then
						...
						if FlightMapFrame then
							...
							for pin in activePool:EnumerateActive() do 
								...
								nodes[#nodes + 1] = {slot = pinSlot, btn = pin}
							end
						else
							for i=1,NumTaxiNodes() do
								local btn = _G["TaxiButton"..i]
								if btn then
									nodes[#nodes + 1] = {slot = i, btn = btn}
								end
							end
						end

						for _, pin in pairs(nodes) do 
							local pinSlot = pin.slot
							local x = taxiNodePositions[pinSlot].x;
							local y = taxiNodePositions[pinSlot].y;

							local loopNPC = DGV.Modules.TaxiData:GetLookupTable()[cont][DGV:PackXY(x,y)]

							if loopNPC and waypoint.flightMasterID==loopNPC then
								TryTakeTaxiNode(pinSlot)

								if pin.btn.Icon then
									HighlightTaxiIconWithRed(pin.btn.Icon)
								end

								if pin.btn.GetNormalTexture and pin.btn:GetNormalTexture() then
									HighlightTaxiIconWithRed(pin.btn:GetNormalTexture())
								end

								if not pin.btn.hooledOnLeave then
									if pin.btn.HookScript then
										pin.btn:HookScript("OnLeave", function()
											HighlightFlightmasterDestination()
										end)
									end
									pin.btn.hooledOnLeave = true
								end
							end
						end
	...
	function DGV:TAXIMAP_OPENED()
		...
		for index = 1, NumTaxiNodes() do
			...
			if type ~= "NONE" then
				local x, y = TaxiNodePosition((button and button:GetID()) or index);
				taxiNodePositions[index].x = x;
				taxiNodePositions[index].y = 1.0 - y;
			end
		end
		...
		for i=1,NumTaxiNodes() do
			...
			elseif nodeType=="REACHABLE" and GetNumRoutes(i)==1 then
				...
				local sX = taxiNodePositions[srcSlot].x;
				local sY = taxiNodePositions[srcSlot].y;
                UpdateLookupTable(cont, sX, sY)

				local dstSlot = TaxiGetNodeSlot(i, 1, false);
				local dx = taxiNodePositions[dstSlot].x;
				local dy = taxiNodePositions[dstSlot].y;
			...
			elseif nodeType=="REACHABLE" then
				...
				for j=1, GetNumRoutes(i) do
					local dstSlot = TaxiGetNodeSlot(i, j, false);
					local dx = taxiNodePositions[dstSlot].x;
					local dy = taxiNodePositions[dstSlot].y;
		...
		if recalulateRoute or (DGV:IsModuleLoaded("Guides") and DGV.actions[DugisGuideUser.CurrentQuestIndex] == "f") then
			...
			UIErrorsFrame:AddMessage("Flight master data updated!",1,1,0,1)			
